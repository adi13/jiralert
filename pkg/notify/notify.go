...
// Notify translates alert to a ticket in following flow:
// *
func (r *Receiver) Notify(data *alertmanager.Data) (bool, error) {
	...
	if issue != nil {
		...
		if issue.Fields.Summary != issueSummary {
			retry, err := r.updateSummary(issue, issueSummary)
			if err != nil {
				return retry, err
			}
		}
...
func (r *Receiver) updateSummary(issue *jira.Issue, summary string) (bool, error) {
	level.Debug(r.logger).Log("msg", "updating issue with new summary", "key", issue.Key, "summary", summary)
	issue.Fields.Summary = summary
	issue, resp, err := r.client.UpdateWithOptions(issue, nil)
	if err != nil {
